...
def unbind(binding_id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            # 3. 提示信息（修复：用 Python 容错后的变量）
            if current_user.id == binding.agent_id:
                flash(f'已成功解除与买家「{buyer_name}」的绑定', 'success')
            else:
                flash(f'已成功解除与代购「{agent_name}」的绑定', 'success')
        except Exception as e:
            db.session.rollback()
...
def bind_buyer_direct_confirm(buyer_id):
    ...
    try:
        db.session.commit()
        flash(f'已直接确认与买家「{buyer_user.username}」的绑定关系', 'success')
